...
# ------------------- START OF SIMPLIFIED DEBUG extract_sequence_features -------------------
def extract_sequence_features(uniprot_data, debug_markdown_list_ref):
    ...
    Simplified debug to check basic loop and access.
    """
    ...
    debug_markdown_list_ref.append("\n\n--- SIMPLIFIED FEATURE EXTRACTION DEBUG ---\n")
    ...
    if "features" in uniprot_data and uniprot_data["features"] is not None:
        ...
        for i, feature_item in enumerate(uniprot_data["features"]):
            if i < 3: # Log only the first 3 raw features to see their structure
                debug_markdown_list_ref.append(f"\n**Raw Feature Index {i}:**\n")
                debug_markdown_list_ref.append(f"```json\n{json.dumps(feature_item, indent=2)}\n```\n")
            ...
            if feature_type in features_of_interest:
                ...
                if begin_pos_val_str is None and end_pos_val_str is None and "position" in location_obj and isinstance(location_obj["position"], dict) and "value" in location_obj["position"]:
                ...
                if begin_pos_val_str and end_pos_val_str:
                    ...
                    except: # Broad except for simplicity in this debug step
                        pass # Silently ignore parsing errors for now
            # --- End of core logic ---
        debug_markdown_list_ref.append(f"Finished iterating through raw features. Extracted count: {len(extracted_features)}\n")
    else:
        debug_markdown_list_ref.append("'features' key not found or is None in uniprot_data.\n")
        
    debug_markdown_list_ref.append("--- END OF SIMPLIFIED FEATURE EXTRACTION DEBUG ---\n")
    return extracted_features
# ------------------- END OF SIMPLIFIED DEBUG extract_sequence_features -------------------
...
def get_protein_info(uniprot_id):
    ...
    try:
        ...
        # ... (Rest of the protein info extraction - keep as is) ...
        ...
        # Simplified Debug call
        seq_features = extract_sequence_features(uniprot_api_data, markdown_parts) 
        
        markdown_parts.append(f"\n\n--- GENERAL DEBUG INFO (after simplified extract) ---\n")
        markdown_parts.append(f"**Final Extracted features count:** {len(seq_features)}\n")
        if seq_features:
            extracted_feature_types_counts = Counter([f['type'] for f in seq_features])
            markdown_parts.append(f"**Final Extracted feature types counts:** {dict(extracted_feature_types_counts)}\n")
...
iface = gr.Interface(
    ...
    title="Protein Profile Viewer (v0.4.4 - Simplified Debug)", 
    description="Enter a UniProt ID. Simplified debug for features is active.",
